...
const sectionTitle = computed(() => String(sectionConfig.value?.title ?? "").trim() || props.defaultTitle || t("cvPreview.education"));
...
<template>
  <section class="pb-6">
    ...
    <div v-else-if="!items.length" class="card-subtle mb-4 flex items-center justify-between gap-2">
      <p class="text-sm text-slate-500">{{ t("cv.emptyEducation") }}</p>
      <button type="button" class="btn-secondary py-1.5 text-xs" @click="addItem">+ {{ t("cvPreview.add") }}</button>
    </div>

    <div v-for="(item, index) in items" v-else :key="index" class="card-subtle mb-4 grid gap-4 sm:grid-cols-2">
